...
const SearchModel: React.FC<SearchModel> = ({ onClose,products,Focused  }) => {
  ...
  return (
        <div className="  fade-in-15  fixed md:mx-auto z-50 inset-0 backdrop-blur-lg shadow-2xl/40  flex  md:w-[60%] h-[50%] mt-[38%] md:mt-[12.1406245%]   overflow-auto overflow-x-hidden bg-slate-100/60 dark:bg-dark dark:shadow-gray-800 " onMouseLeave={onClose} >                  
        ...
        <div className=" mx-auto px-5  fade-in w-full gap-4"  > 
                ...
                {products && products.length>0 ? (
                        products.map((product) => (
                                <Link key={product.id} href={`/category/${encodeURIComponent(String(product.category))}`} onClick={onClose}  className='flex gap-2' >
